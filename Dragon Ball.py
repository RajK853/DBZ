	...
	def add(self, time):       # adds new object on the screen according to the time.
		...
		if time%1000 == 0:              # makes planets on the screen in every 1000 frames
			if (time/1000) <= 3:         # the first three planets have relatively smaller radius and less life i.e easier to destroy
				...
				self.objects.append({"rect" : pygame.Rect(WINW, 0, self.PLANETRADIUS+(5*int(time/1000)), self.PLANETRADIUS+(5*int(time/1000))), "speed" : self.PLANETSPEED, "life" : [2*self.PLANETLIFE+int(time/1000)*10, 2*self.PLANETLIFE+int(time/1000)*10], "image" : pygame.transform.scale(pygame.transform.rotate(pygame.image.load("Data/Image/Planet.png"), 20), (self.PLANETRADIUS+(5*int(time/1000)), self.PLANETRADIUS+(5*int(time/1000))))})
...
class Energy:
	...
	def add(self, char):       # Adds new energy dictionary to the list energy
		...
		if char.superSayan:
			energyImg = pygame.transform.scale(pygame.image.load("Data/Image/SS_Energy.png"), (self.ENERGYW, self.ENERGYH))
		...
		# choose energy blast's image when super sayan mode is off
			...
			energyImg = pygame.transform.scale(pygame.image.load("Data/Image/Energy.png"), (self.ENERGYW, self.ENERGYH))
...
class Character:
	...
	def blink(self, dir):             # makes the player blink up or down
		if self.superSayan:
			blinkImg = pygame.transform.scale(pygame.image.load("Data/Image/SS_Blink.png"), self.size)
		else:
			blinkImg = pygame.transform.scale(pygame.image.load("Data/Image/Blink.png"), self.size)
	...
	def playerImage(self, task, setRect):                  # return player's image accoring to the current task it is doing
		# choose among the Super Sayan's images is super sayan mode on
		if self.superSayan:
			if self.type == "hero":
				if task == "None":
					img = pygame.image.load("Data/Image/SS_Normal.png")
				elif task == "Shoot":
					img = pygame.image.load("Data/Image/SS_Shooting.png")
				elif task == "Charge":
					img = pygame.image.load("Data/Image/SS_Charging.png")
				elif task == "Release":
					img = pygame.image.load("Data/Image/SS_Releasing.png")
				elif task == "Release Wave":
					img = pygame.image.load("Data/Image/SS_Releasing_e.png")
		# choose among the normal images if super sayan mode off
		else:
			if self.type == "hero":
				if task == "None":
					img = pygame.image.load("Data/Image/Normal.png")
				elif task == "Shoot":
					img = pygame.image.load("Data/Image/Shooting.png")
				elif task == "Charge":
					img = pygame.image.load("Data/Image/Charging.png")
				elif task == "Release":
					img = pygame.image.load("Data/Image/Releasing.png")
				elif task == "Release Wave":
					img = pygame.image.load("Data/Image/Releasing_e.png")
		self.Img = pygame.transform.scale(img, self.size)
		if setRect: # tell to get player rect
...
def drawHealthBar(character):      #  health bar with current health left
	barImg = pygame.transform.scale(pygame.image.load("Data/Image/Energy_bar.png"), (ENERGYBARW, ENERGYBARH))
	if character.type == "hero":
		pygame.draw.rect(windowSurface, GREEN, (12, WINH - ENERGYBARH - 23, character.health, ENERGYBARH-2))
		windowSurface.blit(barImg, pygame.Rect(10, WINH - ENERGYBARH - 25, ENERGYBARW, ENERGYBARH))
	else:
		pygame.draw.rect(windowSurface, GREEN, (WINW-character.health-12, WINH - ENERGYBARH - 23, character.health, ENERGYBARH-2))
		windowSurface.blit(barImg, pygame.Rect(WINW-character.health-10, WINH - ENERGYBARH - 25, ENERGYBARW, ENERGYBARH))

def drawEnergyBar(character, manualDrain):                     # draw energy bar with current amount of energy left in it
	barImg = pygame.transform.scale(pygame.image.load("Data/Image/Energy_bar.png"), (ENERGYBARW, ENERGYBARH))
	pygame.draw.rect(windowSurface, RED, (12, WINH - 18, character.energy, ENERGYBARH-2))
	windowSurface.blit(barImg, pygame.Rect(10, WINH - 20, ENERGYBARW, ENERGYBARH))
	if character.charging[0]:                    # if kamehameha is charging
		character.energy -= manualDrain        # decrease the energyBar with value given for manualDrain
		if character.energy < 0:
			character.energy = 0
	else:                           # if kamehameha not charging
		if character.superSayan:         # if SS mode on
			if character.energy > 0:       # if energy not zero
				character.energy -= Energy().ENERGYPERSECOND        # drain energy in SS mode
		else:                       # is SS mode off
			if character.energy < MAXBARVALUE:           # if energy not full
				character.energy += Energy().ENERGYPERSECOND            # increase the energy
		if character.energy == 0	or character.energy == MAXBARVALUE:           # if energy zero or full
		...
		for event in pygame.event.get():
			...
			if event.type == KEYDOWN:
				if event.key == ord("w"):
					...
					if hero.energy >= 50:             # if energy is atleast 50
						hero.charging = [True, "k"]             # charging kamehameha
						stopAllSounds()
						SOUND.charging.play()
						drawEnergyBar(hero, 50)       # drain energy by 50
				elif event.key == ord("d") and "explosive wave" in hero.abilities:
					if hero.energy >= MAXBARVALUE:             # if energy is full
						hero.charging = [True, "e"]             # charging
						stopAllSounds()
						SOUND.charging_e.play()
						drawEnergyBar(hero, MAXBARVALUE)       # drain all the energy
